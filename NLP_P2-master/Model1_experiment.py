...
def hmm(transitionDict, emisssionDict, unknown_pro, unknown_tran_pro, labels, test, weight):
        ...
        for i in range(l):
            ...
            for j in range(l):
                ...
                if (test[t]+"|"+labels[i]) not in emisssionDict:
                    emisssionDict[test[t]+"|"+labels[i]] = unknown_tran_pro
                temp = score[j][t-1] + weight[labels[i]+"|"+labels[j]] * np.log(transitionDict[labels[i]+"|"+labels[j]])\
                       + np.log(emisssionDict[test[t]+"|"+labels[i]])
                if temp > maximum:
                    maximum =temp
                    score[i][t] = temp
                    bptr[i][t] = j
...
if __name__ == "__main__":
    ...
    with open('Model1_Prediction.csv', mode='w') as report_file:
        ...
        for test in test_word_original:
            result = hmm(label_trans_dic_pro, pro_word_label, 0.000000001, 0.000000001, ['0', '1'], test, weight)
            for label in result:
