...
def hmm(transitionDict, emisssionDict, labels, test):
    ...
    for i in range(l):
        ...
        score[i][0] = np.log(transitionDict[test[0]+"|"+labels[i]]) + np.log(emisssionDict[labels[i]+"|"+test[0]])
        bptr[i][0] = 0
    ...
    for t in range(1, len(test)):
        for i in range(l):
            ...
            for j in range(l):
                ...
                temp = score[j][t-1] + np.log(transitionDict[test[t]+"|"+labels[i]]) + np.log(emisssionDict[labels[i]+"|"+test[t]])
                if temp > maximum:
                    maximum = temp
                    score[i][t] = temp
                    bptr[i][t] = j
...
if __name__ == "__main__":
    ...
    with open('test_model2_3wordFeature_validation_subTransition.csv', mode='w') as report_file:
