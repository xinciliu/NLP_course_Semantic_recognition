...
def hmm(emisssionDict, labels, test):
    l = len(labels)
    ...
    for i in range(l):
        score[i][0] = np.log(emisssionDict[labels[i]+"|"+test[0]])
        bptr[i][0] = 0
    ...
    for t in range(1, len(test)):
        for i in range(l):
            maximum = float("-inf")
            for j in range(l):
                temp = score[j][t-1] + np.log(emisssionDict[labels[i]+"|"+test[t]])
                if temp > maximum:
                    maximum = temp
                    score[i][t] = temp
                    bptr[i][t] = j
...
if __name__ == "__main__":
    ...
    with open('test_model2_3wordFeature_validation_subAll.csv', mode='w') as report_file:
        ...
        for test in test_word_original:
            ...
            result = hmm(label_word_pro, ['0', '1'], test)
            for label in result:
