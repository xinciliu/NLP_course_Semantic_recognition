            ...
            if len(word)==1:  #if only one word exist
                ...
                if i==len(word)-1:
                    ...
                    if preSetLabel:
                        ...
                        label = labellist[i-1]
            pos_window.append({'pos-1':tag[0], 'pos':tag[1], 'pos+1':tag[2], 'tag-1':label})
    return pos_window
...
def hmm(transitionDict, emisssionDict, labels, test):
    ...
    for i in range(l):
        if test[0]+"|"+labels[i] not in transitionDict:
            transitionDict[test[0] + "|" + labels[i]] = 0.0000000001
        score[i][0] = np.log(transitionDict[test[0]+"|"+labels[i]]) + np.log(emisssionDict[labels[i]+"|("+test[0]+",0)"])
        bptr[i][0] = 0
    ...
    for t in range(1, len(test)):
        for i in range(l):
            maximum = float("-inf")
            for j in range(l):
                if test[t]+"|"+labels[i] not in transitionDict:
                    transitionDict[test[t] + "|" + labels[i]] = 0.0000000001
                temp = score[j][t-1] + np.log(transitionDict[test[t]+"|"+labels[i]]) + np.log(emisssionDict[labels[i]+"|("+test[t]+","+labels[j]+")"])
                if temp > maximum:
                    maximum = temp
                    score[i][t] = temp
                    bptr[i][t] = j
...
if __name__ == "__main__":
    ...
    pro_word_label, _ = preprocessing(train_file)
    ...
    with open('test_model2_3tagLastLabelFeature_validation_subTransition.csv', mode='w') as report_file:
        ...
        for test in test_word_original:
            ...
            result = hmm(pro_word_label, label_word_pro, ['0', '1'], test)
            for label in result:
